...
export function registerAuthoringRoutes(app:App){
  ...
  app.post(base,async c=>{
    ...
    await stagePdf(c.env,storageKey,bytes);
    const valid='EXISTS(SELECT 1 FROM documents d JOIN document_edit_locks l ON l.document_id=d.id WHERE d.id=? AND d.current_version_id=? AND d.is_deleted=0 AND l.user_id=? AND l.token=? AND l.expires_at>?)';
    try{await c.env.DB.batch([
      c.env.DB.prepare(`INSERT INTO versions(id,document_id,storage_key,original_name,mime_type,size,sha256,extracted_text,created_by,created_at) SELECT ?,?,?,?,?,?,?,?,?,? WHERE ${valid}`).bind(versionId,d.id,storageKey,d.name,'application/pdf',bytes.length,sha,sourceText(source),user.userId,now,d.id,b.versionId,user.userId,token,Date.now()),
