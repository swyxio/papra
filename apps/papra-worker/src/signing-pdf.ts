...
export function validateSigningText(value: string) {
  if (!textFont.canEncode(value))
    throw new Error(
      'This text contains characters outside the supported signing font. Use a supported name or signature.',
    );
}
...
export async function digestBytes(value: Uint8Array) {
  return Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256', value)), (x) =>
...
export async function signingPdf(bytes: Uint8Array) {
  ...
  if (
    pdf
      .getForm()
      ?.getSignatureFields()
      .some((x) => x.getSignatureDict())
  )
    throw new Error('This PDF already contains a digital signature; upload an unsigned original');
  const pages = pdf.getPages();
  if (!pages.length || pages.length > SIGNING_MAX_PAGES)
    throw new Error('Signing supports PDFs with 1 to 100 pages');
...
export function validateFields(input: unknown, recipients: number, pages: number): SigningField[] {
  if (!Array.isArray(input) || input.length < 1 || input.length > 100)
    throw new Error('Add 1 to 100 signing fields');
  const fields = input.map((f, i) => {
    if (
      !f ||
      !['signature', 'name', 'date', 'text'].includes(f.type) ||
      ...
      throw new Error('Place each field inside a PDF page and assign it to a recipient');
    return {
      id: `field_${i}`,
  ...
  for (let i = 0; i < recipients; i++)
    if (!fields.some((f) => f.recipient === i && f.type === 'signature'))
      throw new Error('Every recipient needs a signature field');
  return fields;
}
export function signingTimeZone(value: unknown) {
  if (value !== undefined && typeof value !== 'string')
    throw new Error('Choose a valid signing time zone');
...
export function formatSigningDate(timestamp: number, timeZone: string) {
...
export async function sealSigningPdf(
  bytes: Uint8Array,
  fields: SigningField[],
  recipients: PdfSigner[],
  audit: { requestId: string; name: string; sourceSha256: string; createdAt: number },
  p12: Uint8Array,
  passphrase: string,
) {
    ...
    const value =
      field.type === 'signature'
        ...
        : field.type === 'name'
          ? recipient.name
          : field.type === 'date'
            ? formatSigningDate(recipient.signedAt, recipient.values._signingTimeZone || 'UTC')
            : recipient.values[field.id];
    const page = pages[field.page - 1],
      box = page.getCropBox();
    const W = page.rotation % 180 ? box.height : box.width,
      H = page.rotation % 180 ? box.width : box.height;
    const vx = field.x * W + 4,
      vy = (1 - field.y - field.height) * H + Math.max(3, field.height * H * 0.3);
    // Invert the PDF viewer's clockwise rotation into PDF user coordinates.
    const point =
      page.rotation === 90
        ? { x: box.x + box.width - vy, y: box.y + vx }
        : page.rotation === 180
          ? { x: box.x + box.width - vx, y: box.y + box.height - vy }
          : page.rotation === 270
  ...
  function line(text: string, isBold = false) {
    ...
    for (const chunk of chunks) {
      ...
      page.drawText(chunk, { x: 45, y, size: isBold ? 14 : 10, font: isBold ? bold : regular });
      y -= isBold ? 25 : 16;
    }
  }
  line('Signing record', true);
  line(audit.name);
  line(`Request: ${audit.requestId}`);
  line(`Original SHA-256: ${audit.sourceSha256}`);
  line(`Sent: ${new Date(audit.createdAt).toISOString()}`);
  y -= 12;
  for (const r of recipients) {
    line(`${r.name} <${r.email}>`, true);
    line(`Signed: ${new Date(r.signedAt).toISOString()}`);
    line(
      `Date signed: ${formatSigningDate(r.signedAt, r.values._signingTimeZone || 'UTC')} (${r.values._signingTimeZone || 'UTC'})`,
    );
    line(`Signature: ${r.signature}`);
    line(`Network address: ${r.address}`);
    line('Identity: possession of the signing link; email identity was not independently verified');
    y -= 12;
  }
  line(
    'Each signer consented to electronic signing and adopted the entered name as their signature.',
  );
  const signer = await P12Signer.create(p12, passphrase, { buildChain: false });
  const result = await pdf.sign({
    signer,
    reason: 'Electronically signed through swyx Drive',
    location: 'https://drive.swyx.io',
